...
const badgeColors: Record<string, string> = {
  significant: "bg-emerald-500/20 text-emerald-300 border-emerald-500/40",
  medium: "bg-blue-500/20 text-blue-300 border-blue-500/40",
  small: "bg-slate-500/20 text-slate-300 border-slate-500/40",
};
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="container mx-auto px-4 py-12 space-y-20">
            ...
            {resourceCategories.map((category) => {
              ...
              return (
                <Link
                  ...
                  href={category.href}
                  className={cn(
                    "group relative overflow-hidden rounded-xl border bg-slate-900/40 p-6 backdrop-blur-sm transition-all duration-300",
                    "hover:-translate-y-1",
                    colors.border,
                    colors.glow
                  )}
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-2 rounded-lg border border-slate-700/50 bg-slate-800/30 text-slate-400 hover:text-white hover:bg-slate-700/50 transition-colors"
                aria-label="Previous"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredResources.map((resource, index) => {
              ...
              const typeColors: Record<string, string> = {
                ...
                news: "bg-amber-500/20 text-amber-300 border-amber-500/40",
                doc: "bg-blue-500/20 text-blue-300 border-blue-500/40",
              };
              ...
              return (
                <Link
                  key={resource.id}
                  href={resource.link}
                  className="group rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-slate-600 hover:shadow-lg"
                >
                  <span
                    className={cn(
        ...
        <section className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-8 md:p-12 backdrop-blur-sm">
          <h2 className="text-2xl font-bold text-white mb-6 text-center">
            About EIPsInsight
          </h2>
          <p className="text-slate-300 text-center max-w-3xl mx-auto mb-8 leading-relaxed">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {team.map((member) => (
              <div
                key={member.name}
                className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm text-center"
              >
                <div className="rounded-full bg-gradient-to-br from-cyan-500/20 via-emerald-500/20 to-purple-500/20 w-20 h-20 mx-auto mb-4 flex items-center justify-center text-2xl font-bold text-white border border-slate-600">
                ...
                <div className="flex items-center justify-center gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-2 py-0.5 text-xs font-medium text-emerald-300 border border-emerald-500/40">
        ...
        <section>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-3">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {grant.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium bg-slate-700/50 text-slate-300 border border-slate-600/30"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-white bg-slate-800/50 border border-slate-700/50 rounded-lg hover:bg-slate-700/50 hover:border-slate-600 transition-colors"
            >
              <Github className="h-4 w-4" />
              Contribute on GitHub
            </a>
            <button className="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-white bg-cyan-500/20 border border-cyan-500/40 rounded-lg hover:bg-cyan-500/30 hover:border-cyan-400 transition-colors">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {partners.map((partner) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group rounded-xl border border-slate-700/50 bg-slate-900/40 p-6 backdrop-blur-sm hover:border-slate-600 transition-all hover:shadow-lg"
              >
                <p className="text-lg font-semibold text-white group-hover:text-cyan-300 transition-colors flex items-center gap-2">
                  {partner.name}
                  <ExternalLink className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                </p>
              </a>
            ))}
            <button className="rounded-xl border border-dashed border-slate-600 bg-slate-900/20 p-6 text-slate-400 hover:text-white hover:border-slate-500 transition-colors">
