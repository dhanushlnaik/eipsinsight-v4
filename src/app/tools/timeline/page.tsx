    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950">
      ...
      <div className="border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 py-3">
          ...
          <h1 className="text-xl font-bold text-white">EIP Status & Commit Timeline</h1>
          <p className="text-xs text-slate-400 mt-0.5">Full lifecycle of any EIP — status changes, category shifts, deadlines, and linked PRs.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl space-y-6">
        ...
        <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-5 backdrop-blur-sm">
          <label className="text-sm font-medium text-slate-400 mb-2 block">Enter an EIP / ERC / RIP number</label>
          <div className="flex gap-3">
            ...
            <button onClick={() => handleSearch()} disabled={loading || !query.trim()}
              className="inline-flex items-center gap-2 px-6 py-3 text-sm font-medium text-white bg-cyan-500/20 border border-cyan-500/40 rounded-lg hover:bg-cyan-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-5 backdrop-blur-sm">
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-bold text-white mb-1">EIP-{data.eipNumber}</h2>
                  <p className="text-base text-slate-300">{data.title ?? "Untitled"}</p>
                  <p className="text-sm text-slate-400 mt-1">Author: {data.author ?? "Unknown"}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {data.type && <span className="text-xs px-2 py-1 rounded-md bg-slate-700/50 text-slate-300 border border-slate-600/30">{data.type}</span>}
                  {data.category && <span className="text-xs px-2 py-1 rounded-md bg-slate-700/50 text-slate-300 border border-slate-600/30">{data.category}</span>}
                </div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5 pt-5 border-t border-slate-700/50">
                <div><p className="text-xs text-slate-500">Created</p><p className="text-sm text-white">{data.createdAt ?? "—"}</p></div>
                <div><p className="text-xs text-slate-500">Repo</p><p className="text-sm text-white uppercase">{data.repo}</p></div>
                <div><p className="text-xs text-slate-500">Status Changes</p><p className="text-sm text-white">{data.statusEvents.length}</p></div>
                <div><p className="text-xs text-slate-500">Linked PRs</p><p className="text-sm text-white">{data.linkedPRs.length}</p></div>
              </div>
            </div>

            {/* Unified Timeline */}
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-5 backdrop-blur-sm">
              <h3 className="text-lg font-semibold text-white mb-6">Full Timeline</h3>
              <div className="relative">
                <div className="absolute left-[19px] top-0 bottom-0 w-0.5 bg-slate-700/50" />
                <div className="space-y-4">
                  {timeline.length === 0 ? (
                    <p className="text-slate-400 text-sm pl-14">No events recorded</p>
                      ...
                      <div key={i} className="flex gap-4">
                        ...
                        <div className="flex-1 pt-1.5">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            ...
                            {event.meta && (
                              <span className="text-[10px] text-slate-500 font-mono bg-slate-800/50 px-1.5 py-0.5 rounded">{event.meta}</span>
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700/50 bg-slate-800/20">
                        ...
                        <th className="px-4 py-3 text-right text-xs font-medium text-slate-400">Commits</th>
                        <th className="px-4 py-3 text-right text-xs font-medium text-slate-400">Files</th>
                    ...
                    <tbody>
                      {data.linkedPRs.map((pr) => (
                        <tr key={pr.prNumber} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                          ...
                          <td className="px-4 py-3 text-slate-200 max-w-[200px] truncate">{pr.title ?? "—"}</td>
                          <td className="px-4 py-3 text-slate-400 text-xs">{pr.author ?? "—"}</td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-slate-500 text-xs">{pr.createdAt ?? "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {!data && !loading && !error && (
          <div className="text-center py-16">
            ...
            <p className="text-lg text-slate-400">Search for an EIP to explore its full timeline</p>
            <p className="text-sm text-slate-500 mt-1">Every status change, commit, and PR — in chronological order</p>
          </div>
        )}
      </div>
    </div>
