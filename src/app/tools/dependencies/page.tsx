      ...
      <div className="border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 py-3">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              <h1 className="text-xl font-bold text-white">EIP Dependency Graph</h1>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 text-xs bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 w-44"
                />
              </div>
              {searchEip && (
          ...
          <>
            ...
            {selectedNode && (
              <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4 backdrop-blur-sm">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setSelectedNode(null)} className="text-xs text-slate-400 hover:text-white flex items-center gap-1">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs text-slate-400">{selectedNode.repo.toUpperCase()}</span>
                  <span className="text-xs text-slate-400">{connectedEips.length} connected EIP{connectedEips.length !== 1 ? "s" : ""}</span>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {connectedEips.map((n) => (
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded-lg border border-slate-700/50 bg-slate-800/30 text-slate-300 hover:text-white hover:border-cyan-500/30 transition-colors"
                      >
            ...
            {Object.keys(nodesByStatus).length > 0 ? (
                ...
                {Object.entries(nodesByStatus).map(([status, statusNodes]) => (
                  <div key={status}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs text-slate-500">{statusNodes.length}</span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                      ...
                      {statusNodes.length > 50 && (
                        <p className="text-xs text-slate-500 flex items-center justify-center">+{statusNodes.length - 50} more</p>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <div className="text-center py-16">
                ...
                <p className="text-lg text-slate-400">No relationships found</p>
                <p className="text-sm text-slate-500 mt-1">Try a different filter or search for a specific EIP</p>
              </div>
            )}
          </>
        )}
