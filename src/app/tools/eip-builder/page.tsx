...
import {
  ArrowLeft, Download, Copy, Check, Import, Loader2, Search,
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 flex flex-col">
      ...
      <div className="border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm sticky top-0 z-40">
        <div className="container mx-auto px-4 py-2">
          <NextLink href="/tools" className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors mb-1.5">
            <ArrowLeft className="h-3.5 w-3.5" />Tools
          </NextLink>
          <div className="flex flex-wrap items-center gap-2">
            {/* New / Import toggle */}
            <div className="flex rounded-lg border border-slate-700/50 overflow-hidden">
              <button onClick={() => setTabMode("new")} className={cn("px-3 py-1.5 text-xs font-medium transition-colors", tabMode === "new" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white")}>
                New {repoType.toUpperCase()}
              </button>
              <button onClick={() => setTabMode("import")} className={cn("px-3 py-1.5 text-xs font-medium transition-colors border-l border-slate-700/50", tabMode === "import" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-white")}>
              ...
              <div className="flex items-center gap-1.5">
                <div className="relative">
                  <input type="text" value={importQuery} onChange={(e) => setImportQuery(e.target.value)}
                    ...
                    className="pl-3 pr-8 py-1.5 text-xs bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 w-40" />
                  <button onClick={handleImport} disabled={importing} className="absolute right-1.5 top-1/2 -translate-y-1/2">
                    {importing ? <Loader2 className="h-3.5 w-3.5 animate-spin text-cyan-400" /> : <Search className="h-3.5 w-3.5 text-slate-400 hover:text-white" />}
                  </button>
                </div>
            ...
            <div className="flex rounded-lg border border-slate-700/50 overflow-hidden ml-auto">
              {(["eip", "erc", "rip"] as EipType[]).map((r) => (
                <button key={r} onClick={() => setRepoType(r)}
                  className={cn("px-3 py-1.5 text-xs font-medium transition-colors", r !== "eip" && "border-l border-slate-700/50", repoType === r ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")}>
                  {r.toUpperCase()}s
                </button>
              ))}
            </div>
            ...
            {/* View mode */}
            <div className="flex rounded-lg border border-slate-700/50 overflow-hidden">
              <button onClick={() => setViewMode("split")} className={cn("px-2.5 py-1.5 text-xs transition-colors", viewMode === "split" ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")} title="Split">
                <Columns2 className="h-3.5 w-3.5" />
              </button>
              <button onClick={() => setViewMode("edit")} className={cn("px-2.5 py-1.5 text-xs transition-colors border-l border-slate-700/50", viewMode === "edit" ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")} title="Edit">
                <Pencil className="h-3.5 w-3.5" />
              </button>
              <button onClick={() => setViewMode("output")} className={cn("px-2.5 py-1.5 text-xs transition-colors border-l border-slate-700/50", viewMode === "output" ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")} title="Output">
      ...
      <div className={cn("flex flex-1 overflow-hidden", viewMode === "split" ? "flex-row" : "flex-col")}>
        {/* LEFT: Editor */}
        {showEdit && (
          <div className={cn("overflow-y-auto p-4 space-y-3", viewMode === "split" ? "w-1/2 border-r border-slate-800/50" : "w-full")}>
            ...
            <div className="rounded-lg border border-slate-700/50 bg-slate-900/40 p-4">
              <h3 className="text-sm font-semibold text-white mb-3">Preamble</h3>
                  ...
                  <div key={field.key} className={cn(field.wide ? "md:col-span-2" : "")}>
                    ...
                    {field.type === "select" ? (
                      <select
                        ...
                        className={cn("w-full px-3 py-2 text-sm bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-200 focus:outline-none focus:border-cyan-500/50", field.disabled && "opacity-40 cursor-not-allowed")}>
                      ...
                      <input type="text" value={data[field.key]} onChange={(e) => handleInput(field.key, e.target.value)}
                        ...
                        className={cn("w-full px-3 py-2 text-sm bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50", field.disabled && "opacity-40 cursor-not-allowed")} />
                    )}
                  </div>
                ))}
              </div>
            ...
            {SECTION_DEFS.map((sec) => {
              ...
              return (
                <div key={sec.key} className="rounded-lg border border-slate-700/50 bg-slate-900/40 overflow-hidden">
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4">
                      ...
                      <textarea
                        ...
                        className="w-full px-3 py-2 text-sm bg-slate-800/50 border border-slate-700/50 rounded-lg text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 resize-y font-mono"
                      />
                    </div>
                  )}
                </div>
        ...
        {/* RIGHT: Output */}
        {showOutput && (
          <div className={cn("overflow-y-auto bg-slate-950/50 flex flex-col", viewMode === "split" ? "w-1/2" : "w-full")}>
            {/* Toggle: Code / Preview */}
            <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-2.5 border-b border-slate-800/50 bg-slate-900/40 backdrop-blur-sm">
              <div className="flex rounded-lg border border-slate-700/50 overflow-hidden">
                <button onClick={() => setPreview(false)} className={cn("px-3 py-1 text-xs font-medium flex items-center gap-1.5 transition-colors", !preview ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")}>
                  <FileCode className="h-3.5 w-3.5" />Code
                </button>
                <button onClick={() => setPreview(true)} className={cn("px-3 py-1 text-xs font-medium flex items-center gap-1.5 transition-colors border-l border-slate-700/50", preview ? "bg-blue-500/20 text-blue-300" : "text-slate-400 hover:text-white")}>
                  <BookOpen className="h-3.5 w-3.5" />Preview
                </button>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={handleCopy} className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs text-slate-300 bg-slate-800/50 border border-slate-700/50 rounded-lg hover:text-white transition-colors">
            ...
            <div className="flex-1 p-4">
              {preview ? (
                ...
                <div className="max-w-2xl mx-auto space-y-4">
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-800/40">
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400">Field</th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-slate-400">Value</th>
                          </tr>
                        </thead>
                        <tbody>
                          {previewTableRows.map(([field, value], i) => (
                            <tr key={i} className="border-t border-slate-800/50 hover:bg-slate-800/20">
                              <td className="px-4 py-2 text-xs font-medium text-slate-300">{field}</td>
                              <td className="px-4 py-2 text-xs text-slate-400">{value}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="prose prose-invert prose-sm max-w-none rounded-lg bg-slate-900/40 border border-slate-700/50 p-6">
                    {previewBody.split("\n").map((line, i) => {
                      if (line.startsWith("## ")) return <h2 key={i} className="text-lg font-bold text-white mt-6 mb-2 first:mt-0">{line.replace("## ", "")}</h2>;
                      if (line.startsWith("### ")) return <h3 key={i} className="text-base font-semibold text-white mt-4 mb-1">{line.replace("### ", "")}</h3>;
                      if (line.startsWith("# ")) return <h1 key={i} className="text-xl font-bold text-white mt-6 mb-2">{line.replace("# ", "")}</h1>;
                      if (line.startsWith("> ")) return <blockquote key={i} className="border-l-2 border-cyan-500/40 pl-3 text-slate-400 italic">{line.replace("> ", "")}</blockquote>;
                      if (line.startsWith("- ")) return <li key={i} className="text-sm text-slate-300 ml-4 list-disc">{line.replace("- ", "")}</li>;
                      if (line.startsWith("---")) return <hr key={i} className="border-slate-700/50 my-4" />;
                ...
                <textarea
                  ...
                  className="w-full h-full min-h-[600px] px-4 py-3 text-sm bg-slate-900/40 border border-slate-700/50 rounded-xl text-slate-300 font-mono leading-relaxed focus:outline-none focus:border-cyan-500/50 resize-none"
                  spellCheck={false}
                />
              )}
            </div>
      ...
      {/* ─── Bottom Action Bar ─── */}
      <div className="border-t border-slate-800/50 bg-slate-900/30 backdrop-blur-sm px-4 py-3">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            {validated && <span className="inline-flex items-center gap-1.5 text-xs text-emerald-400"><CheckCircle2 className="h-3.5 w-3.5" />Validated</span>}
            {validationErrors.length > 0 && !validated && (
              <button onClick={() => setShowValidationDialog(true)} className="inline-flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleValidate} disabled={validating}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-500/20 border border-blue-500/40 rounded-lg hover:bg-blue-500/30 transition-colors disabled:opacity-50">
            ...
            <button onClick={handleDownload} disabled={!validated}
              className={cn("inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors",
                validated ? "text-white bg-cyan-500/20 border border-cyan-500/40 hover:bg-cyan-500/30" : "text-slate-500 bg-slate-800/30 border border-slate-700/30 cursor-not-allowed"
              )}>
      ...
      {showValidationDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowValidationDialog(false)}>
          <div className="bg-slate-900 border border-slate-700/50 rounded-xl shadow-2xl max-w-lg w-full mx-4 max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/50">
              ...
              <button onClick={() => setShowValidationDialog(false)} className="text-slate-400 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
              ...
              {validationErrors.map((err, i) => (
                <div key={i} className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm text-red-300 font-medium">{err.summary}</p>
                      {err.detail && err.detail !== err.summary && (
                        <p className="text-xs text-slate-400 mt-1">{err.detail}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="flex justify-end px-5 py-3 border-t border-slate-700/50">
              <button onClick={() => setShowValidationDialog(false)}
                className="px-4 py-2 text-sm font-medium text-white bg-red-500/20 border border-red-500/40 rounded-lg hover:bg-red-500/30 transition-colors">
                Close
              </button>
            </div>
          </div>
